...
def test_framing_and_cors_are_allowlists_that_refuse_a_wildcard() -> None:
    """A wildcard is refused by SHAPE, not by a list of spellings somebody remembered.

    The refusal matched a fixed set of exact tokens, so `https://*.bank.example` was in none of
    them, was accepted, and was emitted verbatim into `frame-ancestors`. CSP honours a host-source
    wildcard, so every subdomain could frame the console, including one obtained by subdomain
    takeover or serving user content, and the same string was accepted into the tenant allowlist
    that is echoed back as `Access-Control-Allow-Origin`. Proved properly by
    `ui/tests/embed-policy.test.mjs`; held to the wall here, from the command a developer actually
    runs before committing, which needs no JavaScript toolchain.
    """
    policy = (UI / "lib" / "embed-policy.mjs").read_text(encoding="utf-8")
    ...
    code = _code_only(policy)
    assert "WILDCARD_TOKENS" in code, "nothing rejects a wildcard origin"
    assert "isWildcard" in code, (
        "there is no wildcard predicate, so each caller decides for itself what a wildcard is"
    )
    # The union of the two halves. The token set alone missed every embedded asterisk; the
    # substring test alone would miss `null`, which is a wildcard by behaviour rather than by
    # spelling because a sandboxed iframe presents a null origin.
    assert 'entry.includes("*")' in code, (
        "the predicate matches exact tokens only, so a host-source wildcard is accepted again"
    )
    assert '"null"' in code, "the null origin is no longer refused by name"
    # Refused, not skipped. A matched wildcard used to `continue`, so the entry vanished and the
    # rest of the list was served: the operator was never told the value selected nothing.
    assert "throw new ConfiguredEmptyError" in code.split("function parseAllowlist")[-1], (
        "parseAllowlist drops a wildcard silently instead of refusing it"
    )
