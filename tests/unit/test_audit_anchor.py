...
from soc_fraud_fusion.adapters.local.audit import (
    LocalAuditAdapter,
)
...
from soc_fraud_fusion.domain.correlation_engine import (
    CorrelationEngine,
)
from soc_fraud_fusion.domain.fusion_service import (
    FusionService,
)
from soc_fraud_fusion.factory import (
    build_fusion_service,
)
from soc_fraud_fusion.packs import (
    attack_map_for,
)

from tests.fixtures import sample_cases

_ANCHOR_ENV = "FRAUDFUSION_AUDIT_ANCHOR"
...
def test_an_append_after_truncation_cannot_relaunder_the_anchor(tmp_path: Path) -> None:
    ...
    # Drive the same truncated audit adapter through the real service, so one ordinary append
    # after the truncation must refuse rather than silently re-anchoring the shortened store.
    container = build_container(settings)
