...
from soc_fraud_fusion.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from soc_fraud_fusion.config import Settings, _read_settings_file
from soc_fraud_fusion.ports.identity import (
    VERIFIED,
...
_API_MODULE = "soc_fraud_fusion.api.app"
...
_DATA_PORTS: tuple[tuple[str, str, str], ...] = (
    ("alerts", "alerts:LocalAlertFeed", "alerts:OnPremAlertFeed"),
    ("audit", "audit:LocalAuditAdapter", "audit:OnPremAuditAdapter"),
    ("generation", "generation:LocalGeneration", "generation:OnPremGeneration"),
    ("grounding", "grounding:LocalGrounding", "grounding:OnPremGrounding"),
    ("retrieval", "retrieval:LocalRetrieval", "retrieval:OnPremRetrieval"),
    ("review_router", "review_router:LocalReviewRouter", "review_router:OnPremReviewRouter"),
    ("safety", "safety:LocalSafety", "safety:OnPremSafety"),
)
...
def _managed_app(monkeypatch: pytest.MonkeyPatch, tmp_path: Any, audience: str | None) -> Any:
    ...
    supplies it. ``None`` UNSETS the variable, which is how "this deployment configured no
    audience" is expressed: the loader now REFUSES a variable that is set and empty, so blanking
    it would be a different (and louder) failure, tested separately below.
    """
...
def test_an_emptied_audience_variable_refuses_at_load_rather_than_resolving_to_empty(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Any
) -> None:
    """The other half of the split: UNSET is "nobody configured one", EMPTIED is a refusal.

    The audience reaches the adapter through ``${VAR:-}`` in the settings file, which now goes
    through the same ``setting_or_default`` the adapters use. An operator who emptied the
    variable gets a boot failure naming it rather than a service that verifies against nothing.
    """
    path = tmp_path / "settings.yaml"
    path.write_text(_REBOUND_SETTINGS + "\n", encoding="utf-8")
