...
def test_the_demo_data_is_obviously_fictional() -> None:
    ...
    cases = (demo.ATO_CASE, demo.MULE_CASE, demo.INJECTION_CASE, demo.PII_CASE)
    scopes = {case.scope for case in cases}
    alerts = [alert for scope in scopes for alert in ALERTS[scope]]
    details = [alert.detail for alert in alerts]  # the raw alert text the demo fuses
    blob = (
        " ".join(details) + " ".join(a.indicator for a in alerts) + demo.ACTOR + demo.PLANTED_NRIC
    )
    for detail in details:
        assert "FICTIONAL" in detail, detail + " does not announce itself as fake"
