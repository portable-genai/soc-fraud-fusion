...
def test_an_env_var_set_to_empty_does_not_inherit_the_files_default(
    ...
    ``${VAR:-default}`` is ``setting_or_default(VAR, default)`` written in YAML, so it answers the
    emptied state the same way: it raises. Resolving to empty instead would make
    ``${VAR:-fallback-region}`` with ``VAR=""`` indistinguishable from ``${VAR:-}``, handing the
    service a value nobody reviewed.
    """
