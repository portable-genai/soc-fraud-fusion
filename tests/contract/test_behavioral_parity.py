...
from soc_fraud_fusion.adapters._review_payload import (
    result_to_review,
)
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    ...
    service = build_fusion_service(build_container(local_settings()))
    result = service.fuse(
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    result = build_fusion_service(local).fuse(
    ...
    onprem = build_fusion_service(build_container(local_settings(profile="onprem")))
    with pytest.raises(NotImplementedError):
        onprem.fuse(sample_cases.ESCALATING_CASE, actor=sample_cases.ACTOR, tenant=FIXTURE_TENANT)
