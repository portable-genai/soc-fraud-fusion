# ARCHITECTURE: SOC Fraud Fusion Copilot (G5)

Hexagonal ports-and-adapters. A pure-stdlib domain core speaks only to ports (`typing.Protocol`s);
adapter families implement them; one env var (`FRAUDFUSION_PROFILE`) swaps the
...
## Layout (`src/soc_fraud_fusion/`)
- `domain/` : pure stdlib, no cloud/framework imports. `kernel.py` (vertical-neutral types,
  `StrEnum` taxonomies from the commons), `models.py` (the incident artifacts), `pii.py` (the
  jurisdiction pattern selection + order), `correlation_engine.py` (the deterministic scoring and
  ATT&CK-mapping engine) and `fusion_service.py` (the orchestrator). `packs.py` (outside `domain/`,
  because it reads YAML) loads `rulepacks/attack_map.yaml` into a frozen `AttackMap`.
- `ports/` : `@runtime_checkable` Protocols (`AlertFeedPort`, `RetrievalPort`, `GroundingPort`,
  `SafetyPort`, `GenerationPort`, `AuditSinkPort`, `ReviewRouterPort`; identity uses the commons
  `IdentityPort`), re-exported once with the `PORT_PROTOCOLS` map. `identity.py` adds
...
## Request pipeline (`FusionService.fuse`, then the caller)
fetch raw cited alerts (`AlertFeedPort`) -> redact + safety-screen the input (`SafetyPort`) BEFORE
the model -> **deterministic correlation, ATT&CK mapping and scoring** (`CorrelationEngine`) ->
