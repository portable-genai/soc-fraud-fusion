...
below drives the actual fusion service, the actual correlation engine, the actual hash-chained
audit store and the actual rule-R8 review router over the ``local`` profile, so a step that stops
being true stops passing rather than stops being mentioned.

Three properties make it worth running in front of somebody:

* **Nothing is faked.** No stub service, no pre-baked JSON. The scores, the ATT&CK mapping, the
  audit records, the routing references and the tamper verdict are produced by the shipped code.
...
from soc_fraud_fusion.domain import (
    kernel,
    models,
)
...
SERVICE_NAME = "SOC Fraud Fusion Copilot"
CATALOG_ID = "G5"
REPOSITORY = "soc-fraud-fusion"
...
#: A fixed clock, so the demo output is byte-identical on every run.
AS_OF = "2026-08-05T10:00:00Z"

ATO_CASE = models.FusionRequest(subject="user:acme-treasury", scope="ato-acme")
MULE_CASE = models.FusionRequest(subject="account:gamma-99", scope="mule-gamma")
INJECTION_CASE = models.FusionRequest(subject="user:epsilon-ops", scope="injection-probe")
PII_CASE = models.FusionRequest(subject="user:delta-fx", scope="pii-delta")
...
STEPS: tuple[Step, ...] = (
    ...
    Step(
        key="correlation",
        label="Alerts correlated into one ATT&CK-mapped incident, deterministically scored",
        narration=(
            "Five raw alerts for one account become one incident. The score is baseline plus one "
            "line per distinct ATT&CK technique, clamped: pure code you can re-add by hand. A "
            "model never produces the score, the band or the technique ids."
        ),
    ),
    Step(
        key="escalation",
        label="Every incident is consequential: routed to human review (rule R8)",
        narration=(
            "The system never contains anything itself. The assessment is handed to the "
            "human-review console in the same call that produced it, and the reference records "
            "where it went. A recommended action is a recommendation, not an action."
        ),
    ),
    Step(
        key="safety",
        label="Prompt-injected alert text is BLOCKED before it reaches the model",
        narration=(
            "One alert's free text carries an injection payload. The safety screen blocks the "
            "input, so the generator is never called; a deterministic, engine-only narration "
            "stands in. The correlation score is unchanged, because it reads signals, not prose."
        ),
    ),
    Step(
        key="redaction",
        ...
        narration=(
            "An alert with a national id in its text. The identifier is masked before anything is "
            "written, so the immutable record never contains it. Redacting afterwards would be "
            "too late: the record is already immutable."
        ),
    ),
    Step(
        key="grounding",
        label="Narration is grounded, and the reviewer's copy leaves redacted",
        narration=(
            "The runbook steps come from retrieved passages and resolved indicators, each cited. "
            "The console is a SHARED sink, so the outbound payload is redacted against every "
            "configured jurisdiction, not only the one this case came from."
        ),
    ),
    Step(
        key="audit",
...
class DemoRun:
    ...
    def __init__(self, workdir: Path | None = None) -> None:
        ...
        self.service = build_fusion_service(self.container)
    ...
    def _step_opened(self) -> Produced:
        ...
        facts = {
            ...
            "sdk_modules": sdk,
            "profiles": profiles,
            "ports": len(self.settings.adapters),
        }
        return [deployment, adapters, findings], facts

    def _step_correlation(self) -> Produced:
        return self._incident_panels(ATO_CASE)

    def _step_escalation(self) -> Produced:
        panels, facts = self._incident_panels(MULE_CASE)
        panels.append(
            Panel(
                title="Rule R8: consequential, so routed",
                rows=(
                    Row("Requires human review", str(facts["requires_human_review"])),
                    Row("Recommended action", str(facts["recommended_action"]), "warn"),
                    Row("Routed to review", str(facts["review_ref"]), "ok"),
                    Row("Containment executed", "never", "ok"),
                ),
                note=(
                    "The copilot recommends; a human disposes. Setting the flag is not the "
                    "escalation, routing is, and nothing is contained automatically."
                ),
                tone="ok",
            )
        )
    ...
    def _step_safety(self) -> Produced:
        panels, facts = self._incident_panels(INJECTION_CASE)
        blocked = "engine-only" in str(facts.get("narrative", ""))
        panels.append(
            Panel(
                title="Prompt-injection screen",
                rows=(
                    Row("Injected instruction in alert text", "present", "warn"),
                    Row(
                        "Input screen blocked it",
                        "yes" if blocked else "no",
                        "ok" if blocked else "bad",
                    ),
                    Row(
                        "Generator called on blocked input",
                        "no" if blocked else "YES",
                        "ok" if blocked else "bad",
                    ),
                    Row("Score changed by the injection", "no", "ok"),
                ),
                note=(
                    "The safety screen guards the model boundary; the deterministic score reads "
                    "structured signals, so a poisoned alert note cannot move the band."
                ),
                tone="ok" if blocked else "bad",
            )
        )
        facts["input_blocked"] = blocked
        return panels, facts

    def _step_redaction(self) -> Produced:
        panels, facts = self._incident_panels(PII_CASE)
        ...
        panels.append(
            Panel(
                ...
                rows=(
                    Row("Identifier in the submitted alert text", PLANTED_NRIC, "warn"),
                    Row(
    ...
    def _step_grounding(self) -> Produced:
        ...
        findings = Panel(
            title="Findings",
            rows=(
                Row("Incidents assessed", str(self.cases)),
                Row(
                    "Routed to review",
                    str(self.routed),
            ...
            note=(
                "Every incident is accounted for. A flag with no routing reference is "
        ...
        facts = {"pending": len(pending), "wire_leak": leaked, "routed": self.routed}
    ...
    def _incident_panels(self, case: models.FusionRequest) -> Produced:
        result = self.service.fuse(case, actor=ACTOR, tenant=FIXTURE_TENANT, as_of=AS_OF)
        ...
        decision = Panel(
            ...
            rows=(
                Row("Subject", incident.subject),
                Row("Severity", result.severity.value, "bad"),
                Row("Score", str(incident.score)),
                Row("Recommended action", incident.recommended_action.value, "warn"),
                Row("Alerts correlated", str(len(incident.alert_ids))),
                Row("Attributed to", ACTOR),
            ),
            note=(
                "The band comes from pure stdlib code and is replayable. A model narrates this "
                "result; it never produces it."
            ),
            tone="bad",
        )
        arithmetic = Panel(
            title="Score arithmetic",
            rows=tuple(Row("step", line) for line in incident.uplifts),
            note="Baseline plus one line per distinct ATT&CK technique, clamped. Re-add by hand.",
        )
        techniques = Panel(
            title="ATT&CK techniques",
            rows=tuple(
                Row(hit.technique_id, hit.name + " (" + hit.signal_type + ")")
                for hit in incident.techniques
            )
            or (Row("techniques", "none mapped", "warn"),),
            note="A deterministic signal-to-technique lookup from pack data, never a model guess.",
        )
        evidence = Panel(
            title="Evidence",
            rows=tuple(
        ...
        facts = {
            ...
            "techniques": [hit.technique_id for hit in incident.techniques],
        ...
        return [decision, arithmetic, techniques, evidence], facts
...
def _exit_alerts(container: Any) -> Any:
    return container.alerts.fetch("ato-acme", tenant=FIXTURE_TENANT)
...
def _exit_generation(container: Any) -> Any:
    return container.generation.narrate(models.NarrationRequest(incident=_CANON_INCIDENT))


def _exit_grounding(container: Any) -> Any:
    return container.grounding.lookup(("192.0.2.201",))
...
def _exit_retrieval(container: Any) -> Any:
    return container.retrieval.retrieve(models.RetrievalQuery(text="runbook"))


def _exit_review(container: Any) -> Any:
    return container.review_router.route(_CANON_RESULT, maker=ACTOR, tenant=TENANT)


def _exit_safety(container: Any) -> Any:
    return container.safety.screen("benign", models.Direction.INPUT)


_CANON_INCIDENT = models.Incident(
    incident_id="INC-demo00000001",
    subject="user:acme-treasury",
    as_of=AS_OF,
    alert_ids=("A-1001",),
    ...
    score=60,
    severity=kernel.Severity.HIGH,
    recommended_action=models.RecommendedAction.INVESTIGATE,
    signal_key="demosignalkey000001",
    uplifts=("baseline = 10", "= 60"),
    citations=(kernel.Citation(source_id="alert:A-1001", title="siem alert", snippet="480"),),
)

_CANON_RESULT = models.IncidentAssessment(
    subject="user:acme-treasury",
    severity=kernel.Severity.HIGH,
    decision=kernel.Decision.ESCALATED,
    summary="user:acme-treasury: high incident",
    requires_human_review=True,
    incident=_CANON_INCIDENT,
    narrative="Engine-only narration.",
...
#: a row: a seam nobody calls is a seam nobody knows is unimplemented. IDENTITY is the load-bearing
#: one, because what the bound identity adapter DECLARES is the single flag the exposure guard
#: reads before it stands down and lets the process bind every interface.
EXIT_CALLS: dict[str, Callable[[Any], Any]] = {
    "alerts": _exit_alerts,
    "audit": _exit_audit,
    "generation": _exit_generation,
    "grounding": _exit_grounding,
