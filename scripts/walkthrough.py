...
def _check_correlation(state: dict[str, Any]) -> list[str]:
    facts = _facts(state)
    problems: list[str] = []
    if not facts.get("requires_human_review"):
        problems.append("an incident did not require human review")
    if not facts.get("review_ref"):
        problems.append("rule R8: an incident was flagged but not routed")
    if int(facts.get("score", 0)) <= 0:
        problems.append("the incident scored nothing, so correlation produced no finding")
    if not facts.get("techniques"):
        problems.append("no ATT&CK technique was mapped for a correlated incident")
    return problems
...
def _check_safety(state: dict[str, Any]) -> list[str]:
    facts = _facts(state)
    problems: list[str] = []
    if not facts.get("input_blocked"):
        problems.append("the prompt-injection payload was not blocked at the input")
    if not facts.get("review_ref"):
        problems.append("rule R8: the injected incident was not routed")
    return problems
...
def _check_redaction(state: dict[str, Any]) -> list[str]:
    ...
    if not facts.get("review_ref"):
        problems.append("rule R8: the incident with personal data was not routed")
    return problems


def _check_grounding(state: dict[str, Any]) -> list[str]:
    facts = _facts(state)
    problems: list[str] = []
...
def _check_audit(state: dict[str, Any]) -> list[str]:
    ...
    if int(facts.get("entries", 0)) < 3:
        problems.append("fewer audit records than incidents assessed")
    return problems
