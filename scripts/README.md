...
| `load_demo_book.py` | Loads the shipped fictional alert book (`src/soc_fraud_fusion/data/demo_book/`) into the managed BigQuery dataset, so the deployment correlates the same rows the laptop reads from DuckDB rather than querying a table nothing created. It fills tables and never creates them, so the Terraform must be applied first; it refuses to overwrite a dataset whose own `book_manifest` does not declare what it holds fictional; and `--tenant` is required, because the tenant predicate IS the authorisation and rows stamped with anything else are invisible to every real user. The foreign alert is kept out of that tenant on purpose. `--dry-run DIR` writes the NDJSON it would load and stops, needing no credentials and no `[gcp]` extra. |
...
| `rename_fork.py` | The mechanical half of adopting this repo as a base: rewrites the package name (which is also the console-script name), the `FRAUDFUSION_` environment prefix, the distribution and resource id, and the Terraform `name_prefix` default, in one pass. It prints a plan and writes nothing without `--yes`. The decisions it cannot make for you are the checklist in `docs/ADOPTING.md`. |
