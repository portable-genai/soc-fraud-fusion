"""FastAPI application for SOC Fraud Fusion Copilot (G5).
...
``FRAUDFUSION_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``FRAUDFUSION_S2S_TOKEN`` is wrong in the most dangerous direction:
...
_INSECURE_DEMO_ENV = "FRAUDFUSION_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "FRAUDFUSION_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "FRAUDFUSION_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "FRAUDFUSION_S2S_AUDIENCE"
_API_HOST_ENV = "FRAUDFUSION_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``FRAUDFUSION_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``FRAUDFUSION_PROFILE`` must never be allowed to
    ...
    ``FRAUDFUSION_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "FRAUDFUSION_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "soc-fraud-fusion"
...
_APP_TITLE = "SOC Fraud Fusion Copilot"
...
#
# Note what is NOT in this expression: FRAUDFUSION_S2S_TOKEN. A service
...
@app.post("/v1/fuse", response_model=FuseResponse, tags=["artifacts"])
def fuse(
    request: FuseRequest,
    ...
    result = service.fuse(
        FusionRequest(subject=request.subject, scope=request.scope),
        actor=principal.actor,
        tenant=principal.tenant,
    )
    # The hand-off never fails an already-correlated, already-audited incident; the response
    ...
    return FuseResponse.from_domain(
