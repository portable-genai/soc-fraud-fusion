...
SOC Fraud Fusion Copilot (G5): Correlates fraud and security alerts into cited, ATT&CK-mapped incidents for human review.
Rendered from `hex-service-template`, so it starts at reference parity rather than converging
toward it. Package `soc_fraud_fusion`, environment prefix
`FRAUDFUSION`, region `asia-southeast1`.
...
- `src/soc_fraud_fusion/domain/` is PURE stdlib. No web framework, no cloud SDK,
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `FRAUDFUSION_S2S_TOKEN` may never enter that decision: it authenticates a
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `FRAUDFUSION_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `FRAUDFUSION_PROFILE` (mentioning it in a refusal message is wanted, reading
