...
export default function Home() {
  ...
  const [subject, setSubject] = useState("user:acme-treasury (FICTIONAL)");
  const [scope, setScope] = useState("ato-acme");
  ...
  async function submit(event: React.FormEvent) {
    ...
    setFailed(false);
    try {
      const response = await fetch(API + "/v1/fuse", {
        method: "POST",
        headers: { "Content-Type": "application/json", "X-Dev-Persona": persona },
        body: JSON.stringify({ subject, scope }),
      });
    ...
    <main>
      ...
      <form onSubmit={submit}>
        ...
        <fieldset>
          ...
          <label>
            Scope (selects the seeded alert set: ato-acme, mule-gamma, routine-beta, pii-delta,
            injection-probe)
            <input value={scope} onChange={(event) => setScope(event.target.value)} />
          </label>
          <button type="submit" disabled={busy}>
            {busy ? "Working" : "Fuse this incident"}
          </button>
        </fieldset>
      </form>
